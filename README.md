...
단계마다 산출물 파일을 직접 열어 눈으로 확인한다.
    ...
    python3 ask_embed.py "시간외근로수당 기준 알려줘"   # ④ 질문 → 숫자 → 거리 → 답변
    python3 link.py         # ⑤ 조각 속 인용 → 링크. 이후 ask_embed.py 가 링크를 따라간다
...
키워드 방식 검색(index.py, ask.py)은 부록이다. 같은 질문을 두 방식에 던져 차이를 볼 수 있다.

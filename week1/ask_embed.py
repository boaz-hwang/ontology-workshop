"""
ask_embed.py — 질문을 숫자로 바꿔, 거리가 가까운 조각을 찾아 AI에게 준다.

    python3 ask_embed.py "시간외근로수당 기준 알려줘"
    python3 ask_embed.py --검색만 "…"      AI를 부르지 않고 거리 계산까지만 본다
    python3 ask_embed.py --링크무시 "…"    조각 속 링크를 따라가지 않는다
